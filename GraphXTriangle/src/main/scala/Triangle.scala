...
object Triangle {
	...
	/*	val graph = GraphLoader.edgeListFile(sc,"p2p-Gnutella08.txt")*/
		...
		log.info("Result Triangle Counting")
		//val triangle = graph.triangleCount().vertices
		val triCounts = graph.partitionBy(PartitionStrategy.RandomVertexCut).triangleCount().vertices

		/*println()
		println("Raw triangle counts per vertex: ")
		triCounts.collect.foreach(println)*/

		val triCount = triCounts.map({case (vertexId, count)=> count}).reduce( (count, acc) => count + acc) / 3

		println("triCount = " + triCount)
	}
